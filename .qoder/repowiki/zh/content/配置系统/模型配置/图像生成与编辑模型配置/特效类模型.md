...
- [VideoEffectGenerator.jsx](file://src/components/VideoEffectGenerator.jsx)
...
- **视频特效**：**新增** 专门用于视频特效生成，提供预设模板库和特效模式支持

这些特效功能基于阿里通义实验室的通义万相模型系列，提供了从文生图到图像编辑再到视频特效的完整AI生成解决方案。
...
VideoEffect[VideoEffectGenerator.jsx]
end
App --> BGGen
App --> AITryOn
App --> GeneratorForm
...
**图表来源**
- [App.jsx](file://src/App.jsx#L1-L454)
...
- **视频特效模型**：**新增** 支持首帧特效和人物变装特效，提供丰富的预设模板库
...
**章节来源**
- [models.js](file://src/config/models.js#L232-L307)
...
- [VideoEffectGenerator.jsx](file://src/components/VideoEffectGenerator.jsx#L96-L118)
...
**新增** 视频特效模型提供以下核心能力：
- **首帧特效**：基于首帧图片生成动态视频效果
- **人物变装**：支持汉服、机甲、杂志封面等风格变换
- **预设模板库**：包含7种通用特效、13种单人特效、3种单人/动物特效、3种双人特效
- **模型版本**：支持Turbo版（速度优先）和Plus版（质量优先）
- **模板分类**：根据特效类型自动过滤可用模板

#### 输入要求
- **输入图片**：必须上传，支持PNG/JPG格式
- **特效模板**：必须选择，根据模型类型自动过滤
- **模型选择**：可选择Turbo版或Plus版

#### 输出控制参数
- **模型类型**：首帧特效或人物变装
- **模板类别**：根据模型类型自动切换
- **生成质量**：Turbo版（部分模板）或Plus版（全部模板）

```mermaid
flowchart TD
Start([开始视频特效]) --> CheckImage["检查输入图片"]
CheckImage --> ImageValid{"图片有效?"}
ImageValid --> |否| Error1["错误：缺少输入图片"]
ImageValid --> |是| CheckModel["检查模型类型"]
CheckModel --> ModelValid{"模型类型有效?"}
ModelValid --> |否| Error2["错误：请选择特效模型"]
ModelValid --> |是| FilterTemplates["根据模型过滤模板"]
FilterTemplates --> SelectTemplate["选择特效模板"]
SelectTemplate --> BuildPayload["构建请求负载"]
...
**章节来源**
- [VideoEffectGenerator.jsx](file://src/components/VideoEffectGenerator.jsx#L1-L277)
- [models.js](file://src/config/models.js#L232-L307)
- [models.js](file://src/config/models.js#L1110-L1158)
...
#### 视频特效模板不匹配
- **问题**：选择的模板与模型不兼容
- **解决方案**：根据模型类型自动切换模板类别，选择兼容的模板
- **预防措施**：使用组件内置的模板过滤功能

**章节来源**
- [useTasks.js](file://src/hooks/useTasks.js#L164-L246)
- [fileUpload.js](file://src/utils/fileUpload.js#L149-L181)
- [VideoEffectGenerator.jsx](file://src/components/VideoEffectGenerator.jsx#L65-L93)
...
- **智能优化**：多项性能优化措施提升用户体验
- **扩展性强**：新增视频特效组件进一步完善了特效生态

### 功能特色
- **多样化的特效**：涵盖背景生成、AI试衣、鞋靴模特、**视频特效**等多个领域
- **灵活的参数控制**：丰富的参数选项满足不同需求
- **完善的错误处理**：全面的错误处理和用户反馈机制
- **智能模板过滤**：根据模型类型自动筛选兼容的特效模板

### 最佳实践建议
1. **合理选择模型版本**：根据需求平衡质量和速度
2. **优化输入质量**：提供高质量的输入图像提升生成效果
3. **监控资源使用**：注意文件大小和生成数量的限制
4. **充分利用模板库**：善用预设模板库提高创作效率
5. **定期更新配置**：及时更新模型配置和API端点信息
...
#### 视频特效使用示例
**新增** 1. 上传输入图片（PNG/JPG格式）
2. 选择特效模型（Turbo版或Plus版）
3. 根据模型类型选择合适的特效模板
4. 点击生成按钮等待特效视频完成
...
#### 视频特效参数
- **model**：模型ID（wanx2.1-i2v-plus/turbo或wanx2.1-kf2v-plus）
- **input.img_url**：输入图片URL
- **input.template**：特效模板值
- **parameters.watermark**：水印控制（false）
- **effectType**：特效类型（i2v或kf2v）
- **supportedTemplates**：支持的模板列表（根据模型而定）